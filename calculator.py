...
while True:
    key_state_f2 = get_key_state("f2")
    key_state_f3 = get_key_state("f3")
    if key_state_f2 == "按下":
        ...
        for i in range(0, 4):
            try:
                ...
                if "牧师" in result:
                    ...
                    price = int(result)
                    pro80 = calc(
                        x=money, x_salary=salary, price=price, salary=80, round=round
                    )
                    pro270 = calc(
                        x=money, x_salary=salary, price=price, salary=270, round=round
                    )
                    print(f"检测到牧师，扣除价格，80投资最终将带来{pro80}收益，270投资最终将带来{pro270}收益")
            except:
                print(f"第{i+1}个英雄没检测到")
